...
状态：**概念与已实现的自有host控制**；运行入口与确切覆盖见[第二批native实验](NATIVE_LAB.zh-CN.md)。
它已覆盖本机ABI、VA/RVA、PDB匹配/MVID和局部符号栈；完整跨环境/任意crash分析仍非已验结果。
...
第二批现在提供新编写程序的重复运行、PDB配对和符号栈收据生成器；它们不是历史Match10已经
逐项完成的证据。源、输入与结果都属于这次独立的自有host实验。
