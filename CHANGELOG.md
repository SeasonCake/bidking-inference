# Changelog

This project follows Semantic Versioning. Dates use `YYYY-MM-DD`.

## [Unreleased]

- Add a compact, bilingual three-step 0.3.4 screenshot flow for standby, live bidding,
  and settlement review; move older interfaces into a collapsed historical section.
- Make Simplified Chinese the default repository home page and add linked English docs.
- Add reviewed historical product screenshots with explicit documentation-only licensing.
- Refresh GitHub Actions to the current Node 24-based action releases.
- Add weighted binary calibration diagnostics and leave-one-evidence-out sensitivity.
- Publish a reviewed historical layer: selected real `v0.2.0-hotfix1` source and frozen
  pre-0.2.8 map/item tables, isolated from the maintained package.
- Expand the bilingual home page with an architecture map, public code map, practical
  use cases, and a clearer maintained-core/legacy/private boundary.

## [0.1.0] - 2026-08-29

- Add strict domain-neutral models and interval constraints.
- Add posterior normalization and deterministic Monte Carlo summaries.
- Add exact, interval, approximate, and categorical observations for arbitrary fields.
- Add stable multi-field scoring, bounded joint enumeration, posterior diagnostics,
  record adapters, and weighted-pool simulation.
- Add a multidimensional CLI schema and three medium-core examples.
- Add a synthetic fixture, command-line entry point, tests, and public-boundary checks.
- Add MIT licensing, DCO contribution terms, and companion-project attribution.
- Add installed-package and fresh-clone verification for the first public release.
